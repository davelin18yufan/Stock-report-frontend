...
const MainSector = () => {
  const { posts, setPosts, reports, setReports, currentTab, setCurrentTab } = useMainContext()
  ...
  return(
    <div className="basis-4/5 h-screen overflow-y-auto flex flex-col items-center space-y-4">
      <div className="w-full aspect-square mt-4 sm:flex relative outline outline-2 outline-transparent duration-500 cover cursor-cell" >
        <img src="https://loremflickr.com/500/340/nature" alt="cover" className=" absolute w-full h-full z-[-1] object-cover duration-500"/>
        <div className="w-max mx-auto mt-2">
          <img 
            src={user?.avatar} alt={user?.name} 
            className="w-40 h-40 rounded-full object-cover" />
          <h3 className="mb-2 text-center font-bold text-2xl text-dark-green mt-3 rounded-lg shadow-lg outline outline-2 outline-transparent hover:outline-gray-600 hover:outline-offset-6 bg-slate-300/30 dark:text-gray-400 ">{user?.name}</h3>
        </div>
      ...
      <div className="w-full ">
        ...
        <div className="w-full h-screen overflow-y-auto scrollbar-y">
          ...
          {currentTab === "favorite" && (favoritePosts.map(item => {
            return(
              <PostCard 
                post={item}
                key={item.id}
