...
export const SearchBar = ({onSearch, value, setValue}) => {
  return(
    <div className="w-full p-2 flex mx-auto border-b-4 border-slate-400/25">
      ...
      <h3 className="text-lg ml-4 my-auto sm:text-xl">搜尋股票以查看報告</h3>
    ...
    <div className="w-full h-full">
      ...
      <ul className="w-full p-2 flex flex-wrap overflow-y-auto">
        {filteredItem.map((item, index) => 
          <li className="w-[110px] m-2 truncate outline outline-2 outline-offset-2 outline-cyan-700/70 rounded-sm text-slate-500 bg-card" key={index} onClick={() => go("1")}>{item}</li>
        )}
      </ul>
    </div>
)
