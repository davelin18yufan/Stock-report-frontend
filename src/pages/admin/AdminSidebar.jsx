  ...
  return (
    <nav className={`${menuToggle ? "scale-x-1" : "scale-x-0"} 
      ...
      <ul className="text-dark-green dark:text-neutral-300 flex-col items-start space-y-8 ">
        <li className={`${menuToggle? "opacity-100" : "opacity-0"} hover:text-green-700 dark:hover:text-gray-500 nav-item sm:opacity-100`} onClick={() => go("/admin/list")}>
          <Tooltip title="活動清單" placement="right">
            <FontAwesomeIcon icon="fa-solid fa-newspaper" />
          </Tooltip>
        </li>
        <li className={`${menuToggle? "opacity-100" : "opacity-0"} hover:text-green-700 dark:hover:text-gray-500 nav-item sm:opacity-100`} onClick={() => go("/admin/users")}>
