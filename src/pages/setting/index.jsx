...
const PreviewAvatar = ({previewURL, setFileSrc, fileInputRef, size}) => {
  return (
    <div>
      <img className={`w-${size} h-${size} rounded-full mx-auto object-cover`} src={previewURL} alt="uploading"/> 
  ...
  async function handleSubmit(){
    ...
    try{
      const { success, message } = await editUser(userId, {
        name, email, password, passwordCheck, avatar: fileSrc
      })
      setIsSubmitting(false)
      if (success) {
    ...
    <MainContainer>
      ...
      <div className="flex flex-col sm:flex-row dark:bg-slate-800">
        ...
        <div className="lg:ml-10 lg:w-1/2 ">
          <h2 className="text-center font-bold text-xl mt-2 dark:text-white">個人資料設定</h2>
          <div className="py-4">
            {showErrorMsg ? <p className="text-center text-red-500">&#9785; {errorMsg}</p> : null}
            {showSuccessMsg ? <p className="text-center text-yellow-500">&#9787; {errorMsg}</p> : null}
            ...
            <InputCard label="信箱 Email" placeholder="請輸入你的信箱" type="text" name="email" 
              ...
              disabled={isSubmitting ? true : false}/>
            <InputCard label="密碼 Password" placeholder="請輸入你的密碼" type="password" name="password" 
              onChange={(inputValue) => setPassword(inputValue)}
              disabled={isSubmitting ? true : false}/>
            <InputCard label="再次確認密碼 Confirm password" placeholder="請再次輸入你的密碼" type="password" name="passwordCheck" 
              onChange={(inputValue) => setPasswordCheck(inputValue)}
              disabled={isSubmitting ? true : false}/>
            <div className="mb-2">
              <div className="w-4/5 bg-[#F5F8FA] my-4 px-2.5 mx-auto dark:bg-gray-700">
                <label className="block pb-0.5 text-dark-green dark:text-neutral-300" htmlFor="avatar">大頭照Avatar</label>
                <input placeholder="選擇你的照片" type="file" name="avatar" 
              ...
              <div className="relative w-4/5 mx-auto sm:hidden">
                { fileSrc ? 
                    <PreviewAvatar previewURL={previewURL} setFileSrc={setFileSrc} fileInputRef={fileInputRef} size={24}/>
                    : null
                }
              </div>
            </div>
            <div className="w-4/5 mx-auto relative">
              ...
              {isSubmitting && <svg className="absolute right-1/3 top-1/4 animate-spin h-5 w-5 border-slate-500 border-t-slate-200 rounded-full border-2" viewBox="0 0 24 24"></svg>}
            </div>
          </div>
        </div>
        {/* 中大螢幕 */}
